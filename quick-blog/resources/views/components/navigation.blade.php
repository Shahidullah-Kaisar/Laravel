...
if (auth()->check()) { //বর্তমানে কোনো user লগইন করা আছে কিনা
    $isLogin = true;
}
            ...
            <button id="user-menu-button" type="button"
                ...
                @if ($isLogin)
                    <img class="w-8 h-8 rounded-full"
                        ...
                        alt="user photo">
                @else
                    <a href="{{ route('login.get') }}" class="bg-red-500 px-4 py-2 text-white rounded-sm">Login</a>
                @endif
            </button>
            <!-- Dropdown menu -->
            @if (auth()->check())
                <div id="user-dropdown"
                    class="hidden absolute top-12 right-0 z-50 my-4 text-base list-none bg-white divide-y divide-gray-100 rounded-lg shadow-sm dark:bg-gray-700 dark:divide-gray-600">
                    <div class="px-4 py-3">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        const userMenuButton = document.getElementById('user-menu-button'); // profile button
        const dropdown = document.getElementById('user-dropdown'); // dropdown box

        if (userMenuButton) {
            // profile image এ ক্লিক করলে dropdown show/hide হবে
            userMenuButton.addEventListener('click', function(event) {
                event.stopPropagation(); // prevent bubbling
                dropdown.classList.toggle('hidden');
            });

            // বাইরে ক্লিক করলে dropdown hide হবে
